...
unsigned char *yz_reconstruction(Image *new_img, c_zdimension *z, int i_px){
    unsigned int    y = 0;
    int             new_width = z->z_dec;
    int             width = (int)new_img->columns;
    int             height = (int)new_img->rows;
    PixelPacket     *px;
    
    if (z->yz_pixel_map == 0){
        z->y_dec        = (int)new_img->rows;
        z->x_dec        = (int)new_img->columns;
        z->z_dec        = z->z_dec;
        z->yz_pixel_map = malloc(z->y_dec *  z->z_dec * sizeof(*z->yz_pixel_map));
    }
    px = GetImagePixels(new_img, 0, 0, new_img->columns, new_img->rows);
...
unsigned char *xz_reconstruction(Image *new_img, c_zdimension *z, int i_px){
    unsigned int    x = 0;
    int             width = (int)new_img->columns;
    PixelPacket     *px;
